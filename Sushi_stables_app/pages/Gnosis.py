...
svg_logo = load_svg("sushi_stables_app/gno.svg")
st.markdown(f'<h1 style="font-size: 3rem; font-weight: bold;">{svg_logo} Gnosis Stablecoin Swaps</h1>', unsafe_allow_html=True)
...
st.markdown(' ')
st.markdown("This page shows a deeper dive into the total volume and net inflows/outflows of stablecoins on Sushiswap Gnosis")
...
fig_total_volume = px.bar(df,
                          ...
                          color='symbol',
                          barmode='stack',
                          title='Total Stablecoin Swap Volume on Gnosis per Token',
...
fig_net_stablecoin_volume = px.bar(df,
                                   ...
                                   color='symbol',
                                   #barmode='stack',
                                   title='Net Stablecoin Swap Volume on Gnosis per Token',
...
fig_normalized_volume = px.bar(df_normalized,
                               ...
                               color='symbol',
                               barmode='stack',
                               title='Normalized Monthly Stablecoin Swap Volume on Gnosis per Token',
...
st.write('''
Gnosis Stablecoin swaps are dominated by USDC and USDT. Although USDC had a larger percdent of the total volume when activity was high in late 2021, the two have since evened out and have a nearly identical share of the swap volume on Gnosis. The interesting thing about Gnosis, is a large spike in inflows of PAXG in August 2022 accounting for 75% of the swap volume that month and inflows of $150k to the network.''')
